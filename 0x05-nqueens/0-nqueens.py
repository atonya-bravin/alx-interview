...
def is_safe_to_place_queen(row, col, placed_queens):
    ...
    for queen_row, queen_col in placed_queens:
        if col == queen_col or row - col == queen_row - queen_col or col - queen_col == queen_row - row:
            return False
    return True
...
def solve_nqueens(board_size, current_row, placed_queens):
    ...
    Parameters:
        board_size: An integer that sets the board size and the number of queens.
        current_row: The current row being considered for queen placement.
        placed_queens: The current state of the board represented as a list of queen positions.
    """
    if current_row == board_size:
        # If we have placed queens in all rows (reached the end of the board), print the solution.
        print(placed_queens)
    else:
        for col in range(board_size):
            if is_safe_to_place_queen(current_row, col, placed_queens):
                # If it's safe to place a queen at position (current_row, col), add it to the board.
                ...
                # Backtrack and remove the last queen placement to explore other possibilities.
                placed_queens.pop()
